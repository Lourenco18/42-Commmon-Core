...
def load_function_definitions(path: Path) -> List[FunctionDefinition]:
    ...
    try:
        ...
        print(f"[ERROR] Cannot read function definitions file: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    for i, item in enumerate(raw):
        try:
            ...
            print(f"[ERROR] Function definition #{i} is invalid: {e}", file=sys.stderr)
            sys.exit(1)

    if not definitions:
        print("[ERROR] Function definitions file is empty.", file=sys.stderr)
        sys.exit(1)
...
def load_prompts(path: Path) -> List[PromptEntry]:
    ...
    try:
        ...
        print(f"[ERROR] Invalid JSON in input prompts file: {e}", file=sys.stderr)
        sys.exit(1)
    except OSError as e:
        print(f"[ERROR] Cannot read input prompts file: {e}", file=sys.stderr)
        sys.exit(1)

    if not isinstance(raw, list):
        print("[ERROR] Input prompts file must contain a JSON array.", file=sys.stderr)
        sys.exit(1)
    ...
    for i, item in enumerate(raw):
        try:
            ...
            print(f"[WARNING] Prompt entry #{i} is invalid, skipping: {e}", file=sys.stderr)

    if not entries:
        print("[ERROR] No valid prompt entries found in input file.", file=sys.stderr)
        sys.exit(1)

    return entries


def save_results(results: list, path: Path) -> None:  # type: ignore[type-arg]
